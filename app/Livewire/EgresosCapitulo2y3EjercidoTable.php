...
class EgresosCapitulo2y3EjercidoTable extends Tabla
{
    public $cacheData = [];
    public $dataCompleta = [];
    public $perPage = 6;
    ...
    {
        return view('livewire.egresos-capitulo2y3-ejercido-table');
    }
    ...
    public function edit($id)
    {
        try {
            ...
            Log::error('Ocurrió un error al editar en ejercido del capitulo 2 y 3: ' . $th->getMessage());
    ...
    public function delete($id)
    {
        try {
            ...
            Log::error('Ocurrió un error al eliminar en ejercido del capítulo 2 y 3: ' . $th->getMessage());
        ...
        foreach ($this->cacheData as $key => $movimiento) {
            if ($movimiento['id'] != $id && str_contains($movimiento['area'], $datosSeleccionado['codigoArea']) && str_contains($movimiento['cuenta'], $datosSeleccionado['codigoCuenta']) && $movimiento['mes'] == $datosSeleccionado['mes']) {
                if ($totalImportes == 0) {
                    ...
                    $movimiento['dispibilidad'] = bcsub(bcsub($movimiento['pttoDevengado'], $totalImportes, 2), $movimiento['importe'], 2);
                    $totalImportes += $movimiento['importe'];
                }
    ...
    public function finalizarRegistros()
    {
        ...
        try {
            ...
            $bitacora->bitacora('finalizarRegistros', 'registro o intentó registrar un ejercido del capítulo 2 y 3 con evento: ' . $this->numeroEvento, request());
            DB::beginTransaction();

            foreach ($this->dataCompleta as $movimiento) {
                $movimiento['importe'] = doubleval($movimiento['importe']);
                $interaccionCuentaConceptoPrincipal = InteraccionCuentaConcepto::where('cuenta_id', '=', $movimiento['cuentaId'])->whereIn('concepto_id', [90, 91])
            ...
            $totalRemanente = DB::select('EXEC ImporteTotalCapitulo2y3Ejercido @evento = ?', array($this->numeroEvento))[0]->MontoDelEvento;
            if ($totalRemanente > 0) {
            ...
            DB::rollBack();
            Log::error('Ocurrió un error al finalizarRegistro en ejercido del capítulo 2 y 3: ' . $th->getMessage());
