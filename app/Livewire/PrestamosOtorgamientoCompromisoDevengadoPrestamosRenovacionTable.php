...
class PrestamosOtorgamientoCompromisoDevengadoPrestamosRenovacionTable extends Tabla
{
    ...
    {
        return view('livewire.prestamos-otorgamiento-compromiso-devengado-prestamosRenovacion-table');
    }
        ...
        try{
            if($this->verificarPresupuesto($registro)){    
                $nuevoRegistro = [
                    ...
                    'mes' => $registro['mes'],
                    'movimiento' => 'OTORGAMIENTO COMPROMISO DEVENGADO PRESTAMOS RENOVACION', 
            ...
            foreach ($this->dataCompleta as $key => $registro) {
                if ($registro['id'] == $id) {
                    $datosRegistro = [
                        ...
                        'cuentaAbono' => $registro['cuentaAbonoId'],
                        'mes' => $registro['mes'],
                        'importe' => $registro['importe'],
                        'importeAbono' => $registro['importeAbono'],
                        'pttoEjecutar' => $registro['pttoEjecutar'],
    ...
    public function finalizarRegistros()
    {
        ...
        try{
            ...
            $bitacora->bitacora('finalizarRegistros', 'registro o intentó registrar un compromiso-devengado préstamos renovación del capítulo 7000: '.$this->numeroEvento, request());
            DB::beginTransaction();
            ...
            foreach ($this->dataCompleta as $movimiento)
            {
                ...
                $interaccionCuentaConceptoPrincipal = InteraccionCuentaConcepto::where('cuenta_id', '=', $movimiento['cuentaId'])->whereIn('concepto_id', [94])
                ...
                $interaccionCuentaConceptoAbono = InteraccionCuentaConcepto::where('cuenta_id', '=', $movimiento['cuentaAbonoId'])->whereIn('concepto_id', [94])
