...
class EgresosCapitulo2y3PagadoTable extends Tabla
{
    ...
    public function agregarRegistro($registro)
    {
        ...
        try {
            ...
            Log::error('Ocurrió un error al agregar registro en pagado del capítulo 2000 y 3000: ' . $th->getMessage());
    ...
    public function edit($id)
    {
        try {
            ...
            Log::error('Ocurrió un error al editar en pagado del capítulo 2 y 3: ' . $th->getMessage());
    ...
    public function delete($id)
    {
        try {
            ...
            Log::error('Ocurrió un error al eliminar en pagado del capítulo 2 y 3: ' . $th->getMessage());
            $this->dispatch('mostrarMensaje', mensaje: 'Ocurrió un error al editar, contacte al área de Gobierno Electrónico', tipo: 'error', tiempo: 3000);
        }
    }
    ...
    public function finalizarRegistros()
    {
        ...
        try {
            ...
            $bitacora->bitacora('finalizarRegistros', 'registro o intentó registrar un pagado del capítulo 2 y 3 con evento: ' . $this->numeroEvento, request());
            DB::beginTransaction();
            foreach ($this->dataCompleta as $movimiento) {
                $movimiento['importe'] = doubleval($movimiento['importe']);
                $interaccionCuentaConceptoPrincipal = InteraccionCuentaConcepto::where('cuenta_id', '=', $movimiento['partidaId'])->whereIn('concepto_id', [92, 93])
                ...
                $polizas = [
                    [
                        ...
                        'cuentaRelacionada' => $movimiento['codigoPartida'],
            ...
            $totalRemanente = DB::select('EXEC ImporteTotalCapitulo2y3Pagado @evento = ?', array($this->numeroEvento))[0]->MontoDelEvento;
            if ($totalRemanente > 0) {
                ...
                foreach ($remanentesContables as $poliza) {
                    if ($poliza['tipo_interaccion'] == 'Contable - Abono') {
                        ...
                        foreach ($polizasDevengadoContableCargo as $cuentaCargo) {
                            if ($cuentaCargo->tipo_interaccion == 'Contable - Cargo' && str_contains($poliza['cuentaRelacionada'], $cuentaCargo->cuenta)) {
                                $cuentaCargo->total = $poliza['total'];
            ...
            $importeTotalEvento = DB::select('EXEC ImporteTotalCapitulo2y3Pagado @evento = ?', [$this->numeroEvento]);
            ...
            DB::rollBack();
            Log::error('Ocurrió un error al finalizarRegistro en pagado del capítulo 2 y 3: ' . $th->getMessage());
