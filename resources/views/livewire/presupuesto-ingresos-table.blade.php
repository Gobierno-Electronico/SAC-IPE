<div>
        ...
        <div class="d-flex flex-row">
            ...
            <select class="select_presupuesto rounded-1 shadow-sm border-0 w-25 ms-5" name="Anio" id="Anio"
                ...
                @for ($i = 2024; $i <= $currentYear; $i++)
                    <option @if ($currentYear === $i)
                        selected
                    @endif value="{{ $i }}">Año {{ $i }}</option>
                @endfor
            </select>
        </div>
    </div>
    ...
    <div class="mt-4">
        <button @if ($validado) disabled @endif class="btn btn_primario shadow border-1 mt-3 mt-md-0"id="validarPresupuesto" data-bs-toggle="modal"
            data-bs-target="#confirmModalvalidarPresupuestoInicial" wire:init="init()">Validar presupuesto inicial</button>
    </div>
    <div class="mt-4 d-flex justify-content-between">
        <button @if ($validado) disabled @endif id="borrarPresupuesto" type="button" class="btn btn-danger shadow border-1 mt-3 mt-md-0"
            data-bs-toggle="modal" data-bs-target="#confirmModalborrarPresupuestoInicial">
            Borrar presupuesto inicial
        </button>
        <div>
            <button id="botonGenerarPoliza"
                onclick="generarPolizaPresupuestalIngresos(this)" type="button"
