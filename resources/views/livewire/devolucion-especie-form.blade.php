    ...
    <div class="row">
        <div class="col-3">
            ...
            <select name="selectCuentaContable" id="selectCuentaContable" class="form-select">
                ...
                @foreach (\App\Models\Cuenta::join('clasificador_rubro_ingreso', 'clasificador_rubro_ingreso.Cuenta_contable', '=', 'cuentas.Codigo_cuenta')->select('cuentas.*', 'clasificador_rubro_ingreso.*')->where('cuentas.Descripcion_cuenta', 'LIKE', '%(Por ejecutar)%')->orderBy('cuentas.Codigo_cuenta')->get() as $cuenta)
