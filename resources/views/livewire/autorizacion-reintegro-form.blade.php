    ...
    <div class="row">
        <div class="col-3">
            ...
            <select name="selectCuentaContable" id="selectCuentaContable" class="form-select">
                ...
                @foreach (\App\Models\Cuenta::join('clasificador_rubro_ingreso', 'clasificador_rubro_ingreso.Cuenta_contable', '=', 'cuentas.Codigo_cuenta')->select('cuentas.*', 'clasificador_rubro_ingreso.*')->where('cuentas.Descripcion_cuenta', 'LIKE', '%(Devengado)%')->orderBy('cuentas.Codigo_cuenta')->get() as $cuenta)
                    <option value="{{ $cuenta->Codigo_cuenta }}">
                        {{ $cuenta->Codigo_cuenta . '  ' . $cuenta->Descripcion_cuenta }}</option>
                @endforeach
            </select>

            <label for="selectCuentaCargo" class="form-label mt-3">Cuenta contable de cargo</label>
            <select name="selectCuentaCargo" id="selectCuentaCargo" class="form-select">
                <option value="0">
                    Seleccionar cuenta de cargo</option>
                @foreach (\App\Models\Cuenta::join('clasificador_rubro_ingreso', 'clasificador_rubro_ingreso.Cuenta_contable', '=', 'cuentas.Codigo_cuenta')->select('cuentas.*', 'clasificador_rubro_ingreso.*')->orderBy('cuentas.Codigo_cuenta')->get() as $cuenta)
                    <option value="{{ $cuenta->Codigo_cuenta }}">
                        {{ $cuenta->Codigo_cuenta . '  ' . $cuenta->Descripcion_cuenta }}</option>
                @endforeach
            </select>
